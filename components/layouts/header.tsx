...
import { IconPackages, IconMenu } from "@tabler/icons-react";
...
export function Header() {
  ...
  return (
    <div className="sticky top-0 z-50 border-border border-b border-dotted bg-black/80 backdrop-blur-md">
      <div className="mx-auto flex max-w-7xl items-center justify-between border-border border-r border-l border-dotted px-4 sm:px-8">
        ...
        <Sheet open={mobileMenuOpen} onOpenChange={setMobileMenuOpen}>
          <SheetTrigger asChild>
            <button
              className="md:hidden rounded-full p-2 text-muted-foreground hover:text-foreground hover:bg-emerald-500/10 transition"
          ...
          <SheetContent
            side="right"
            className="w-[280px] sm:w-[320px] border-dotted"
          >
            <SheetHeader>
            ...
            <nav className="mt-8 flex flex-col gap-2">
              {NAV_ITEMS.map((item) => (
                <Link
                  ...
                  className="rounded-lg px-4 py-3 text-base font-medium text-muted-foreground transition hover:text-foreground hover:bg-emerald-500/10"
                >
                  {item.label}
                </Link>
              ))}
            </nav>
            <div className="mt-8 pt-8 border-t border-dotted">
              <div className="flex flex-col gap-3">
                <RegistrySetup />
                <a
                  className={cn(
                    buttonVariants({ variant: "outline" }),
                    "rounded-full w-full justify-center"
                  )}
